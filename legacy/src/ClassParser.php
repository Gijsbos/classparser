...
use gijsbos\ClassParser\Classes\ClassAttribute;
...
abstract class ClassParser
{
     ...
     */
    private static function createClassComponent(ReflectionClass $reflectionClass, string $fileContentNoComments) : ClassComponent
    {
        ...
        $classComponent->extends = self::extractExtends($fileContentNoComments);
        $classComponent->implements = self::extractImplements($fileContentNoComments);
        $classComponent->curlyBracketOnNewline = self::curlyBracketOnNewline($reflectionClass->getStartLine(), $fileContentNoComments);
     ...
     * escape
     */
    private static function escape(string $fileContent) : string
    {
        $fileContent = TextParser::replaceCommentQuote($fileContent, function($match)
        {
        ...
        $fileContent = replace_enclosed("(", ")", $fileContent, "$", "U+0024");
     ...
     * getExtra
     */
    private static function getExtra(string $fileContent) : string
    {
        if(preg_match("/(.*?{{[0]}})(.*)/s", $fileContent, $matches) == 1)
            return $matches[2];
        else
            return "";
    }
    ...
    {
        // Match all
        preg_match_all("/[\t ]*(?!use[\s]+([a-zA-Z0-9\_\\\]+)\s*;)use[\s]+([a-zA-Z0-9\_\\\]+)/", $fileContent, $matches);
    ...
    public static function getUsesDefinitions($className, array $options = [])
    {
        ...
        // Extract
        $fileContent = file_get_contents($className->getFileName());

        // Return
        $usesDefinitions = self::extractUses($fileContent);
     ...
     * createClassObject
     */
    private static function createClassObject(ReflectionClass $reflectionClass, string $filePath, string $escapedFileContents, string $fileContentNoComments)
    {
        // Placeholder replace
        ...
        $classObject->uses = self::extractUses($fileContentNoComments);
     ...
     * parseMethods
     */
    private static function parseMethods(ClassObject $classObject, string $classBody, array $placeholders, string $fileContent, string $escapedFileContents, string $fileContentNoComments) : ClassObject
    {
        // Match all functions
        ...
        foreach($matches[0] as $i => $match)
        {
            ...
            $trailingNewLines = self::getTrailingNewlines($fileContent, $match);
     ...
     * parseUses
     */
    private static function parseUses(ClassObject $classObject, string $classBody, array $placeholders, string $fileContents, string $escapedFileContents, string $fileContentsNoComments) : ClassObject
    {
        // Match all functions
        ...
        foreach($matches[0] as $i => $match)
        {
            ...
            $useName =          $matches[2][$i];
            $inlineComment =    self::unescape(placeholder_restore($matches[3][$i], $placeholders));

            // Create constant
            $classTrait = new ClassTrait($classObject->getName(), $useName);
            $classTrait->setLine(self::getLineInText($fileContents, $match));
            $classTrait->text = $match;

            // Create class component constant
            $classComponent = new ClassComponent(ClassComponent::NEW_USE, $useName);
            $classComponent->indentation = $indentation;
            $classComponent->docComment = "";
            ...
            $classComponent->trailingLineBreaks = substr_count($trailingNewLines, "\n") + 1;
            // Update text
            $classTrait->text .=  $trailingNewLines;
            // Set classComponent
            $classTrait->component = $classComponent;
            // Add constant
            $classObject->uses[$useName] = $classTrait;
            $classObject->component->addComponent($classComponent);
        }
        // Return
        return $classObject;
     ...
     * parseAttributes
     */
    private static function parseAttributes(ClassObject $classObject, string $classBody, array $placeholders, string $fileContents, string $escapedFileContents, string $fileContentsNoComments) : ClassObject
    {
        // Match all functions
        preg_match_all("/\n?([\t ]*)use[\s]+([a-zA-Z0-9\_]+);(.*?)(?=\n)/si", $classBody, $matches);
        ...
        foreach($matches[0] as $i => $match)
        {
            ...
            $match =            self::unescape(placeholder_restore($match, $placeholders));
            $indentation =      $matches[1][$i];
            $attributeName =    $matches[2][$i];
            $inlineComment =    self::unescape(placeholder_restore($matches[3][$i], $placeholders));

            // Create attribute
            $classAttribute = new ClassAttribute($classObject->getName(), $attributeName);
            $classAttribute->setLine(self::getLineInText($fileContents, $match));
            $classAttribute->text = $match;

            // Create class component constant
            $classComponent = new ClassComponent(ClassComponent::NEW_ATTRIBUTE, $attributeName);
            $classComponent->indentation = $indentation;
            $classComponent->docComment = "";
            $classComponent->inlineComment = $inlineComment;
            ...
            // Update text
            $classAttribute->text .=  $trailingNewLines;

            // Set classComponent
            $classAttribute->component = $classComponent;

            // Add constant
            $classObject->attributes[$useName] = $classAttribute;
            $classObject->component->addComponent($classComponent);
        }
        // Return
        return $classObject;
     ...
     * parseConstants
     */
    private static function parseConstants(ClassObject $classObject, string $classBody, array $placeholders, string $fileContent, string $escapedFileContents, string $fileContentNoComments) : ClassObject
    {
        // Match all functions
        ...
        foreach($matches[0] as $i => $match)
        {
            ...
            $classConstant->setLine(self::getLineInText($fileContent, $match));
     ...
     * parseProperties
     */
    private static function parseProperties(ClassObject $classObject, string $classBody, array $placeholders, string $fileContent, string $escapedFileContents, string $fileContentNoComments) : ClassObject
    {
        // Match all functions
        ...
        foreach($matches[0] as $i => $match)
        {
            ...
            $classProperty->setLine(self::getLineInText($fileContent, $match));
     ...
     * parseClassBody
     */
    private static function parseClassBody(ClassObject $classObject, string $fileContent, string $escapedFileContents, string $fileContentNoComments) : ClassObject
    {
        // Set classBody
        $classBody = $classObject->body;
        ...
        // Get placeholders
        $startIndex = 0;
        $placeholders = placeholder_replace("{", "}", $classBody, $startIndex);

        // Parse methods
        $classObject = self::parseMethods($classObject, $classBody, $placeholders, $fileContent, $escapedFileContents, $fileContentNoComments);

        // Parse constants
        $classObject = self::parseUses($classObject, $classBody, $placeholders, $fileContent, $escapedFileContents, $fileContentNoComments);

        // Parse constants
        $classObject = self::parseAttributes($classObject, $classBody, $placeholders, $fileContent, $escapedFileContents, $fileContentNoComments);

        // Parse constants
        $classObject = self::parseConstants($classObject, $classBody, $placeholders, $fileContent, $escapedFileContents, $fileContentNoComments);

        // Parse properties
        $classObject = self::parseProperties($classObject, $classBody, $placeholders, $fileContent, $escapedFileContents, $fileContentNoComments);
    ...
    public static function getBody($reflection) : string
    {
        ...
        // Get function body
        $body = implode("\n", array_slice(explode("\n", $fileContent), $reflection->getStartLine(), $reflection->getEndLine() - $reflection->getStartLine()));
